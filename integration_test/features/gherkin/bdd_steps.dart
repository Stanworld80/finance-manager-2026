...
extension BddSteps on WidgetTester {
  ...
  Future<void> givenTheAppIsRunning() async {
    try {
      ...
      print("Firebase initialized successfully in test!");
    } catch (e, stack) {
      print("Firebase initialization failed in test: $e");
      print(stack);
    }
  ...
  Future<void> whenILogin(String email, String password) async {
    ...
    try {
      ...
      if (find.byType(DashboardScreen).evaluate().isNotEmpty) {
        print("Already logged in (Dashboard found after timeout)!");
        return;
      }
      rethrow;
    }
  ...
  Future<void> whenINavigateToRecurring() async {
    await whenINavigateTo('Échéanciers');
    final manageBtn = find.byTooltip('Gérer les récurrences');
    if (manageBtn.evaluate().isNotEmpty) {
      await tap(manageBtn.first);
      await pumpAndSettle(const Duration(seconds: 2));
    }
  }
  ...
  Future<void> whenINavigateToTransactions() async {
    final voirToutBtn = find.text('Voir tout');
    if (voirToutBtn.evaluate().isNotEmpty) {
      await tap(voirToutBtn.first);
      await pumpAndSettle(const Duration(seconds: 2));
    } else {
      await whenINavigateTo('Transactions');
    }
  }
